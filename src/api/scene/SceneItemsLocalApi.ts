...
class SceneItemsLocalApi {
  ...
  async getItems<ItemType extends Item>(
    ...
    if (Array.isArray(filter)) {
      const { items } = await this.messageBus.sendAsync<{ items: ItemType[] }>(
        "OBR_SCENE_ITEMS_LOCAL_GET_ITEMS",
        { ids: filter },
      );
      return items;
    } else {
      const { items } = await this.messageBus.sendAsync<{ items: ItemType[] }>(
        "OBR_SCENE_ITEMS_LOCAL_GET_ALL_ITEMS",
        {},
      );
  ...
  async updateItems<ItemType extends Item>(
    ...
    await this.messageBus.sendAsync("OBR_SCENE_ITEMS_LOCAL_UPDATE_ITEMS", {
      updates,
      fastUpdate,
    });
  }
  ...
  async addItems(items: Item[]) {
    await this.messageBus.sendAsync("OBR_SCENE_ITEMS_LOCAL_ADD_ITEMS", {
      items,
    });
  }

  async deleteItems(ids: string[]) {
    await this.messageBus.sendAsync("OBR_SCENE_ITEMS_LOCAL_DELETE_ITEMS", {
      ids,
    });
  }
  ...
  async getItemAttachments(ids: string[]): Promise<Item[]> {
    const { items } = await this.messageBus.sendAsync<{ items: Item[] }>(
      "OBR_SCENE_ITEMS_LOCAL_GET_ITEM_ATTACHMENTS",
      { ids },
    );
    return items;
  }
...
export default SceneItemsLocalApi;
